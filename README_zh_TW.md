...
- 按下「下載 MP3 音訊」開始下載時，也會平滑捲到「2 選擇畫質」區塊開頭，與影片下載一致。首次授予儲存權限後也會定位，下載進度更新時不會反覆捲動。
- 沿用原簽章。版本為 1.3.8（versionCode 13）。

## 1.3.7 下載時定位畫面

- 按下「下載影片」開始下載時，畫面平滑捲到「2 選擇畫質」區塊開頭；首次授予儲存權限後也會定位。下載進度更新時不會反覆捲動。
- 保留原簽章與先前的下載修正。版本為 1.3.7（versionCode 12）。

## 1.3.6 修正 Facebook 重新導向循環

- 保留 Facebook 指定且通過驗證的轉址主機，包括 `m.facebook.com`。1.3.5 誤將使用者輸入網址的正規化套用到伺服器轉址，把行動版網址改回 `www.facebook.com`，部分連線因此反覆跳轉。
- 重複的目的網址會立即停止；不同參數連續轉址仍維持六次請求上限。錯誤資訊加入連線階段、合法主機及狀態碼，不包含路徑、分享權杖或 Cookie。
- 重新導向全程保留第一個確認的影片 ID，繼續驗證主機、連接埠、網址帳密及 HTTPS。
- 保留標題命名、登入／18+ 提示及原簽章。版本為 1.3.6（versionCode 11），仍未提供需登入影片的下載功能。

## 1.3.5 Facebook 登入與年齡限制提示

- Facebook 桌面頁未提供影片資料時，再讀取同一支影片的行動版頁面一次；保留影片 ID，拒絕導向不同影片，不以推薦影片替代。
- 辨識明確的登入及 18+ 限制，主畫面顯示「此影片需要 Facebook 登入」及原因。一般登入按鈕或程式碼中的文字不會被當成限制。
- 回報連結 `https://www.facebook.com/share/v/19kggoHxPN/` 導向影片 `2117957879137667`；2026-09-25 的未登入行動版頁面明確要求登入才能觀看 18+ 內容。此版改善判斷與提示，仍無法下載這支受限制影片，尚未新增登入或 Cookie 匯入。
- 保留影片／MP3 使用網頁標題命名，沿用原簽章，版本為 1.3.5（versionCode 10）。

## 1.3.4 使用標題命名下載檔案

- 影片與 MP3 使用分析網頁時取得的標題，例如「海邊旅行.mp4」及「海邊旅行.mp3」；副檔名依實際影片或音訊格式決定。
- 保留中文、空格與 emoji；替換檔名不允許的字元，過長標題會安全截短，無可用標題時使用「V-Get」。
- 重複下載自動加上序號，例如「海邊旅行 (1).mp4」，保留原有檔案；完成訊息顯示 Android 實際儲存的檔名。
- 適用五個平台與兩種下載流程，不會重新命名既有檔案。沿用原簽章，版本為 1.3.4（versionCode 9）。
...
已知需登入的測試連結可加上 `-PvgetLiveExpectedAccess=AGE_RESTRICTED` 或 `LOGIN_REQUIRED`，驗證明確的限制訊息；此模式不嘗試下載影片，一般測試仍不連線至 Facebook。
...
小紅書若導向登入或驗證頁，會提示限制，不會改抓其他筆記。本版本不提供登入或 Cookie 匯入。保留 1.2.1 的固定簽章，目前版本為 1.3.8（versionCode 13）。
